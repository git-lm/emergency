$(function () {
    function getWod(data,readOnly){
        ...
        + '</div>'
        + '<div class="form-group">'
        + '<label class="col-sm-2 control-label" style="padding-right:0px;">简 写：</label>'
        + '<div class="col-sm-10">'
    ...
    $('.view_wod').click(function () {
        ...
        $.post(base + 'train/get_train', {
            ...
            var html = getWod(result.data,'readonly="readonly" ');
    ...
    //删除训练数据
    $('.del_wod').click(function () {
        var train_id = $(this).attr('itemid');
        parent.layer.confirm('您确定要删除吗？', {
            btn: ['已经不需要了', '再考虑一下'] //按钮
        }, function () {
            $.post(base + 'train/del_wod', {
                train_id:train_id
            }, function (data) {
                var result = $.parseJSON(data);
                if (result.success) {
